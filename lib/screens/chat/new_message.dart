...
class NewMessage extends StatefulWidget {
  ...
  // NewMessage(this.conversation_id);
  const NewMessage(
      {Key? key,
      // required this.conversation_id,
      required this.chat_user_id,
      required this.name, this.messageTemplate})
      : super(key: key);
...
class _NewMessageState extends State<NewMessage> {
  ...
  Future<void> _listTemplate() async {
    ...
    //   loading = true;
    // });
    final response = await http.post(Uri.parse(ChatUrl.messageTemplate), body: {
    ...
    if (response.contentLength == 2) {
      ...
      final data = jsonDecode(response.body);
      data.forEach((api) {
        final ab = MessageTemplate(api['id'], api['message']);
        templates.add(ab);
      });
      if (this.mounted) {
        // setState(() {
  ...
  submit() async {
    if (_image == null) {
      // if (_imageFile == null) {
      await getPref();
      final response = await http.post(Uri.parse(ChatUrl.newMessage), body: {
        // "user_id": user_id,
        // "post_user_id": widget.model.post_user_id,
        // "chat_content": chat_content,
        "user_id": user_id.toString(),
        "second_user_id": widget.chat_user_id.toString(),
        "body": body,
      });
      final data = jsonDecode(response.body);
      int value = data['value'];
      String pesan = data['message'];
      int conversation_idAPI = data['conversation_id'];
      if (value == 1) {
        print(value);
        // setState(() {
        //   txtChat.clear();
        ...
        Navigator.push(
            ...
            PageTransition(
                ...
                child: ChatScreen(
                  conversation_id: conversation_idAPI,
                  chat_user_id: widget.chat_user_id,
                  name: widget.name,
                  isNew: true,
                )));
      } else {
      ...
      try {
        ...
        var uri = Uri.parse(ChatUrl.newMessage);
        final request = http.MultipartRequest("POST", uri);
        request.fields['user_id'] = user_id.toString();
        request.fields['second_user_id'] = widget.chat_user_id.toString();
  ...
  void initState() {
    ...
    _listTemplate();
    if(widget.messageTemplate != null){
      setState(() {
        txtChat.text = widget.messageTemplate.toString();
      });
    }
    // loadPosts();
  }
